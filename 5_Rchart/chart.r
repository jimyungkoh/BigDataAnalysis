...
#plot 함수
head(cars)
## type 인자: "p" 점으로, “l”	선으로, “b”	점과 선 둘다 동시에, “o”	점과 선 둘다 동시에 (단 겹쳐짐 : overplotted)
## “h”	히스토그램과 비슷한 형태로 (histogram), “s”	계단모양으로 (stair steps), “S”	계단모양으로 (upper stair steps)
## “n”	좌표찍지 않음
##main 인자: 제목
...
#예제 데이터
df <- read.csv("5_Rchart/data/example_studentlist.csv", stringsAsFactors=T)
str(df)
df$name
...
plot(df$height, df$bloodtype) #x 변수 수치형, y 변수 팩터형: 산점도
...
##fill이나 color 인자에 값을 주는대로 컬러를 구분함
g + geom_bar()
